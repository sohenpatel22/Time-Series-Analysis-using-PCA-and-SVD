...
from load_data import prepare_climate_data
...
def main():
    ...
    yearly_df = prepare_climate_data()
    standardized_df, scaler = standardize_dataframe(yearly_df)

    selected_countries = ["Belgium", "Canada", "Oman", "Spain", "Sudan", "Sweden"]

    print("\nClimate data shape:", yearly_df.shape)
    print("Selected countries:", selected_countries)

    plot_time_series(
        yearly_df,
        selected_countries,
        title="Climate Data: Original Time Series",
        y_label="Temperature",
        save_path="outputs/plots/climate_time_series.png"
    )

    pca_results = run_pca(standardized_df)
    ...
    plot_scree(
        ...
        cumulative_variance,
        title="Climate Data: Scree Plot",
        save_path="outputs/plots/climate_scree.png"
    )
    ...
    plot_first_n_pcs(
        ...
        eigenvectors,
        n_components=16,
        title="Climate Data: First 16 Principal Components",
        save_path="outputs/plots/climate_first_16_pcs.png"
    )

    pca_reconstructed_std = {
        1: reconstruct_dataframe(standardized_df, eigenvectors, 1),
        2: reconstruct_dataframe(standardized_df, eigenvectors, 2),
        4: reconstruct_dataframe(standardized_df, eigenvectors, 4),
        8: reconstruct_dataframe(standardized_df, eigenvectors, 8),
        16: reconstruct_dataframe(standardized_df, eigenvectors, 16),
    }
    ...
    plot_incremental_reconstruction(
        ...
        pca_reconstructed_original,
        selected_countries,
        title="Climate Data: Incremental PCA Reconstruction",
        y_label="Temperature",
        save_path="outputs/plots/climate_incremental_reconstruction.png"
    )
    ...
    plot_residuals(
        ...
        selected_countries,
        title="Climate Data: Residuals for PCA Reconstruction (16 Components)",
        y_label="Residual",
        save_path="outputs/plots/climate_pca_residuals_16.png"
    )
    ...
    plot_rmse_curves(
        pca_rmse,
        title="Climate Data: PCA RMSE vs Number of Components",
        y_label="RMSE",
        save_path="outputs/plots/climate_pca_rmse.png"
    )

    svd_std_reconstructed = reconstruct_svd_dataframe(standardized_df, 16)
    svd_std_reconstructed = inverse_transform_dataframe(svd_std_reconstructed, scaler)

    svd_original_reconstructed = reconstruct_svd_dataframe(yearly_df, 16)
    ...
    plot_comparison_residuals(
        ...
        comparison_reconstructed,
        selected_countries,
        title="Climate Data: PCA vs SVD Residual Comparison",
        y_label="Residual",
        save_path="outputs/plots/climate_pca_vs_svd_residuals.png"
    )
    ...
    for name in selected_countries:
        if name not in yearly_df.index:
            continue

        original_row = yearly_df.loc[name].values.reshape(1, -1)
        rmse_list = []

        for k in range(1, 11):
            reconstructed_df = reconstruct_svd_dataframe(yearly_df, k)
    ...
    plot_comparison_rmse(
        comparison_rmse,
        title="Climate Data: PCA vs SVD RMSE Comparison",
        y_label="RMSE",
        save_path="outputs/plots/climate_pca_vs_svd_rmse.png"
    )
